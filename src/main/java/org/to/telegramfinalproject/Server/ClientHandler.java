...
public class ClientHandler implements Runnable {
    private static final String PROFILE_PICS_DIR = "uploads/profile_pics";
    ...
    public ClientHandler(Socket socket) {
        ...
        try {
            ...
            this.out = new PrintWriter(new OutputStreamWriter(rawOut), true);
            this.bin = new DataInputStream(rawIn);
            this.clientSocket = clientSocket;
        } catch (IOException e) {
            throw new RuntimeException("Stream init failed", e);
        }
    }
...
//                                ));
                    ...
                    case "update_profile_picture":
                        handleProfilePictureUpload(requestJson, clientSocket, out);
                        break;
...
//            for (UUID r : receivers) RealTimeEventDispatcher.sendToUser(r, chatPayload);
    ...
    private void handleProfilePictureUpload(JSONObject request, Socket clientSocket, PrintWriter out) {
        try {
            String userId = request.getString("user_uuid");
            String fileName = request.getString("file_name");
            long fileSize = request.getLong("file_size");
            String mime = request.getString("mime");

            System.out.println("[PP] META => user=" + userId
                    + ", file=" + fileName
                    + ", size=" + fileSize
                    + ", mime=" + mime);

            File uploadDir = new File("uploads/profile_pics");
            if (!uploadDir.exists()) {
                boolean created = uploadDir.mkdirs();
                System.out.println("[PP] Directory created: " + created);
            }

            String savePath = "uploads/profile_pics/" + userId + "_" + fileName;
            try (FileOutputStream fos = new FileOutputStream(savePath)) {
                ...
                System.out.println("[PP] File saved at: " + savePath);
            }

            userDatabase udb = new userDatabase();
            User user = udb.findByUserId(userId);

            JSONObject response = new JSONObject();
            if (user != null) {
                userDatabase.updateUserProfilePicture(user.getInternal_uuid(), savePath);
                user.setImage_url(savePath);
                System.out.println("[PP] DB updated for user: " + userId);

                response.put("status", "success");
                response.put("message", "Profile picture updated.");
                JSONObject data = new JSONObject();
                data.put("image_url", savePath);
                response.put("data", data);
                System.out.println("[PP] Response: SUCCESS, URL=" + savePath);
            } else {
                response.put("status", "error");
                response.put("message", "User not found.");
                System.out.println("[PP] Response: ERROR - User not found");
            }

            out.println(response.toString());
            System.out.println("[PP] Response sent to client: " + response.toString());

        } catch (IOException e) {
            System.err.println("[PP] Error while handling profile picture upload: " + e.getMessage());
            e.printStackTrace();
        } catch (Exception e) {
            System.err.println("[PP] Unexpected error: " + e.getMessage());
            e.printStackTrace();
        }
    }

    // removing irrelevant paths and characters
